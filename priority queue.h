...
void PriorityQueue<ValueType>::enqueue(ValueType value, double priority)
{
    ...
    Cell* ncell = new Cell;
    ncell->data = value;
    ncell->priority = priority;
    // if there is no element in the priorityqueue
    if (isEmpty()) {
        head = ncell;
        ncell->link = nullptr;
        count++;
    }
    // if the priority is higher than the current highest one
    else if (cp->priority > priority) {
        ncell->link = head;
        head = ncell;
        count++;
        ...
        for (int i=0; i< count-1; i++) {
            ...
            if ((cp->priority) <= priority && (cp->link->priority) >= priority) {
                ncell->link = cp->link;
                cp->link = ncell;
                count++;
                break;
        ...
        // if the priority is lower than the current lowest one
        cp->link = ncell;
        count++;
    }
}
...
{
    ValueType result;
    if (isEmpty()) error("dequeue: no element in the PriorityQueue");
    else {
        Cell* cp = head;
        result = cp->data;
        head = cp->link;
...
void PriorityQueue<ValueType>::deepCopy(const PriorityQueue<ValueType> & src)
{
    ...
    for (int i=0; i<src.count; i++){
        enqueue(srcp->data,srcp->priority);
        srcp = srcp->link;
    }
}
